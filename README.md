...
2. Engineers 71 rolling features (form, xG, xA, fixture difficulty, availability) shifted by 1 gameweek to prevent data leakage
...
## Project Structure
    ...
    |   +-- 08_gameweek_pipeline.ipynb      Full gameweek-level pipeline (main)
    |
    +-- src/
    |   +-- features.py                     Feature engineering helper module
    |   +-- optimiser.py                    ILP squad optimiser module
    |
    +-- data/
...
### Squad Optimisation (ILP)
    ...
    Subject to:
      sum(c_i * x_i) <= 100     budget constraint
